...
\begin{equation}
  ...
  \label{eq:acc_eq}
\end{equation}
得られる値は0から1で，1に近いほど正例を正しく予測できている．

\subsection{F1-Score}
F1-Scoreは，PrecisionとRecallの調和平均で計算される指標であり，Confusion Matrixを用いて\eref{eq:f1_eq}，\eref{eq:f1_eq2}と表せる．
\begin{equation}
...
\begin{equation}
  Loss(x,class)=-\rm{log}\Biggl(\frac{\exp(\textit{x}[\textit{class}])}{\sum_{\textit{j}}\rm{exp}(\textit{x}[\textit{j}])}\Biggr)=-\textit{x}[\textit{class}]+log\Biggl(\sum_{\textit{j}}exp(\textit{x}[\textit{j}])\Biggr)
  \label{eq:loss_eq}
\end{equation}
