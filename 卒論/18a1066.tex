...
Attentionスコアを求める例として，「I like an apple, it is tasty.」という文章を考える．
初めに，文字列「I like an apple, it is tasty.」を入力とする．入力は単語ごとにベクトルに埋め込まれ今回は$x_{1},...,x_{9}$と表す．
$x_{1},...,x_{9}, d_{x}=512$はそれぞれ訓練過程で学習された重み行列$W_{Q},W_{K},W_{V}, W_{Q}\in \mathbb{R}, W_{K}\in \mathbb{R},  W_{V}\in \mathbb{R}$と掛け合わせ，それぞれ$q_{1},...,q_{9}, k_{1},...,k_{9}, v_{1},...,v_{9}$とする．
また，
...
\subsection{F1-Score\cite{門脇大輔2019-10-09}}
...
\begin{equation}
  ...
  \label{eq:f1_eq}
\end{equation}
Presisionは正例と予測したもののうち真の値も正例の割合，Recallは真の値が正例のもののうちどの程度を正例の予測として含めることができているかの割合である．
...
\begin{table}[htbp]
  ...
  \label{tb:res50}
  \centering\begin{tabular}{c|ccc}\hline
    \backslashbox{指標}{モデル} & resnet18d & resnet50d\\\hline
